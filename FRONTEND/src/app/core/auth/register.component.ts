...
@Component({
  ...
  styles: [
    `
      ...
      .error { color: #b00020; }
      .muted { color: #675f88; }
      .rules { margin: 4px 0 0; padding-left: 18px; display: grid; gap: 4px; color: #50497a; }
      .pass { color: #0a7f20; }
      .fail { color: #666; }
  ...
  template: `
    <div class="auth-page">
      <div class="wrap">
        <h2>Regisztráció</h2>
        <div class="muted subtitle">A fiókot HR jóváhagyás után lehet használni.</div>
        ...
        <div class="row">
          ...
          <div class="password-wrap">
            <input id="password" [type]="showPassword ? 'text' : 'password'" [(ngModel)]="password" placeholder="Legalább 8 karakter" />
            <button
              class="toggle"
              type="button"
        ...
        <div class="actions dual">
          <button class="btn primary" (click)="register()" [disabled]="busy">Regisztráció</button>
...
export class RegisterComponent {
  ...
  get passwordRules() {
    return evaluatePasswordRules(this.password, this.policy);
  }

  get passwordScore(): number {
    return calculatePasswordStrength(this.password, this.policy);
  }
  ...
  async register(): Promise<void> {
    ...
    if (!email || !password) {
      this.error = 'Az e-mail és a jelszó kötelező.';
      return;
    }
    ...
    this.busy = true;
    try {
      await firstValueFrom(this.http.post(`${API_BASE}/api/auth/register`, { email, password }));
      this.message = 'Sikeres regisztráció. Átirányítás a bejelentkezéshez...';
      setTimeout(() => {
      ...
      } else if (e?.error?.errors && typeof e.error.errors === 'object') {
        const firstEntry = Object.values(e.error.errors)[0] as any;
        this.error = Array.isArray(firstEntry) && firstEntry.length
          ? String(firstEntry[0])
          : 'Regisztráció sikertelen.';
      } else {
        this.error = e?.error?.error ?? 'Regisztráció sikertelen.';
      }
    } finally {
      this.busy = false;
    }
  ...
  generatePassword(): void {
    ...
    this.password = generatePasswordByPolicy(this.policy, targetLength);
  }
